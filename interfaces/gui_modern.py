...
class ModernAIGUI:
    ...
    def _adjust_widget_height_dynamically(self, text_widget):
        ...
        try:
            ...
            # ⚡ CORRECTION: Pas de limite maximale pendant l'animation pour afficher tout le contenu
    ...
    def _apply_final_cleanup_formatting(self, text_widget):
        ...
        try:
            ...
            if remaining_bold:
                print(
                    f"[DEBUG] Formatage final de {len(remaining_bold)} éléments manqués"
                )
                # Utiliser la même méthode que pendant l'animation
                self._apply_smart_formatting_once(text_widget)
            else:
                print("[DEBUG] Aucun formatage manqué, tout est déjà correct")
    ...
    def _insert_markdown_and_links(self, text_widget, text):
        ...
        _prev_state = text_widget.cget("state")
        text_widget.configure(state="normal")
        ...
        for match in re.finditer(combined_pattern, text):
            ...
            if match.group("markdown"):  # Lien Markdown [texte](url)
                ...
                if markdown_match:
                    ...
                    print(
                        f"[DEBUG] Lien Markdown corrigé: texte='{link_text}', url='{url}'"
                    )
                else:
            ...
            if not url or not url.strip() or url == "None":
                print(
                    f"[DEBUG] URL invalide: {repr(url)}, insertion comme texte normal"
                )
                text_widget.insert(
                    "end", link_text if "link_text" in locals() else match.group(0)
                )
                last_end = match.end()
                continue
    ...
    def _insert_complete_markdown_with_code(self, text_widget, text):
        ...
        for m in re.finditer(docstring_pattern, text):
            ...
            if m.start() > last_end:
                self._insert_complete_markdown_with_code_no_docstring(
                    text_widget, text[last_end : m.start()]
                )
            # Docstring
            docstring = m.group(0)
            docstring_clean = re.sub(
                r"^([\"']{3})docstring", r"\1", docstring, flags=re.IGNORECASE
            )
    ...
    def _insert_complete_markdown_with_code_no_docstring(self, text_widget, text):
        ...
        code_matches = list(
            re.finditer(code_pattern, text, flags=re.DOTALL | re.IGNORECASE)
        )
        if not code_matches:
        ...
        for code_match in code_matches:
            if code_match.start() > last_end:
                ...
                link_count += self._process_text_with_links_only(
                    text_widget, text_before, link_count
                )
            ...
            if language == "python":
                ...
                self._insert_javascript_code_block_without_newlines(
                ...
                self._insert_dockerfile_code_block_without_newlines(
                    text_widget, code_content
                )
            elif language == "json":
    ...
    def _insert_simple_markdown_legacy(self, text_widget, text):
        ...
        def parse_segments(text, patterns):
            ...
            for m in re.finditer(pattern, text, flags=re.MULTILINE):
                if m.start() > last:
                    segments.extend(
                        parse_segments(text[last : m.start()], patterns[1:])
                    )
    ...
    def _apply_smart_progressive_formatting(self, text_widget):
        ...
        try:
            ...
            # Appliquer un formatage progressif SIMPLE et EFFICACE
            self._apply_simple_progressive_formatting()
    ...
    def _apply_simple_progressive_formatting(self):
        ...
        print(
            "[INFO] Méthode de formatage legacy désactivée - utilisation du système unifié"
        )
        return
            ...
            for match in re.finditer(pattern, actual_content, flags=flags):
                try:
                    ...
                    if style_base == "title":
                        ...
                        hash_start = self._safe_char_to_tk(
                            actual_content, match.start()
                        )
                        content_start = self._safe_char_to_tk(
                            actual_content, match.start(2)
                        )
                        content_end = self._safe_char_to_tk(
                            actual_content, match.end(2)
                        )
                        ...
                        if hash_start and content_start and content_end:
                            ...
                            if not any(
                                tag.startswith("title") or tag == "hidden"
                                ...
                                # Formater le titre
                                text_widget.tag_add(
                                    tag_name, content_start, content_end
                                )
                    else:
                        ...
                        opening_start = self._safe_char_to_tk(
                            actual_content, match.start()
                        )
                        content_start = self._safe_char_to_tk(
                            actual_content, match.start(1)
                        )
                        content_end = self._safe_char_to_tk(
                            actual_content, match.end(1)
                        )
                        ...
                        if all(
                            [opening_start, content_start, content_end, closing_end]
                        ):
                            # Vérifier si cette zone n'est pas déjà formatée
                            existing_tags = text_widget.tag_names(content_start)
                            if not any(
                                tag in ["bold", "italic", "mono", "hidden"]
                                for tag in existing_tags
                            ):
                                # Masquer la balise d'ouverture
                                text_widget.tag_add(
                                    "hidden", opening_start, content_start
                                )
                                # Formater le contenu
                                text_widget.tag_add(
                                    tag_name, content_start, content_end
    ...
    def _is_already_formatted(self, text_widget, start_tk):
        ...
        try:
            ...
            return any(
                tag in ["bold", "italic", "mono", "hidden"] or tag.startswith("title")
    ...
    def _apply_markdown_formatting(self, text_widget):
        ...
        try:
            ...
            self._apply_progressive_markdown_styles(text_widget)
    ...
    def _apply_progressive_markdown_styles(self, text_widget):
        """Applique les styles markdown sans effacer le contenu"""
        # Utiliser le contenu RÉEL du widget au lieu du paramètre text
        ...
        patterns = [
            ...
            (
                r"^(#{1,6})\s+(.+)$",
                "title_markdown",
                lambda m: (m.group(2), f"title{min(len(m.group(1)), 5)}"),
            ),
            # Gras
        ...
        for pattern, style_name, extract_func in patterns:
            ...
            for match in re.finditer(pattern, actual_content, flags=flags):
                start_pos = self._find_text_position_widget_based(
                    text_widget, match.start()
                )
                end_pos = self._find_text_position_widget_based(
                    text_widget, match.end()
                )

                if start_pos and end_pos:
                    # Extraire le texte et le style
                    tag_style = extract_func(match)
    ...
    def _find_text_position(self, full_text, char_index):
    ...
    def _apply_final_syntax_highlighting(self, text_widget):
        ...
        try:
            ...
            for match in re.finditer(code_block_pattern, content, re.DOTALL):
                ...
                print(
                    f"[DEBUG] Bloc de code trouvé - Langage: {language}, Contenu: {len(code_content)} caractères"
                )
            ...
            for segment_text, segment_style in segments:
                if segment_style == "normal":
                    ...
                    for match in re.finditer(pattern, segment_text):
                        ...
                        if match.start() > pos:
                            new_segments.append(
                                (segment_text[pos : match.start()], "normal")
                            )
    ...
    def quel_handle_message_with_id(self, user_text, request_id):
        ...
        if hasattr(self, "logger"):
            ...
            self.logger.debug(
                "Intent detected: %s (confidence %.2f)", intent, confidence_display
            )

        # 🚀 NOUVEAU: Utiliser CustomAI unifié si disponible
        print(
            f"[DEBUG] (ModernAIGUI) Question transmise - Mode {'CustomAI' if self.custom_ai else 'Standard'} : {repr(user_text)}"
        )
        try:
            if self.custom_ai:
                ...
                try:
                    ...
                    if stats.get("context_size", 0) > 100000:  # Plus de 100K tokens
                        print(
                            f"📊 Contexte après traitement: {stats.get('context_size', 0):,} tokens"
                        )
                except Exception:
                    pass
            else:
        ...
        def run_async_task():
            ...
            try:
                ...
                response = loop.run_until_complete(
                    self.ai_engine.process_query(message)
                )
