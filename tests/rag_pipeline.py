    ...
    def chunk_text(self, text: str, max_chunk_size: int = 2048) -> List[str]:
        ...
        for paragraph in paragraphs:
            ...
            if len(paragraph) > max_chunk_size:
                sentences = self._split_sentences(paragraph, re)
...
class VectorStore:
    ...
    def __init__(self, dimension: int = 384):
        ...
        if FAISS_AVAILABLE:
            ...
            # Inner Product pour similarité cosinus
            ...
            self.vectors = []
            self.vector_ids = []
            print("⚠️ Mode fallback activé (pas de FAISS)")
...
class RAGPipeline:
    ...
    def search(self, query: str, top_k: Optional[int] = None) -> List[str]:
        """Recherche et retourne les textes pertinents"""
        if top_k is None:
    ...
    def retrieve(self, query: str, k: Optional[int] = None) -> List[DocumentChunk]:
    ...
    def generate_response(
        ...
        augmented_context = self._build_augmented_context(retrieved_chunks)
        ...
        rag_context = {
            **context,
            ...
            "augmented_context": context_preview
        }
    ...
    def _build_augmented_context(
        ...
        for i, chunk in enumerate(chunks):
            ...
            source_label = f"Source {i+1} - {doc_name} (sim: {similarity:.2f})"
    ...
    def _build_enhanced_prompt(
        self, original_query: str, augmented_context: str
    ) -> str:
...
Si le contexte contient des informations pertinentes, intègre-les naturellement.
Si le contexte n'est pas pertinent, réponds normalement mais mentionne que tu
n'as pas trouvé d'informations spécifiques dans la base de connaissances."""
    ...
    def save_index(self, save_path: str):
        ...
        if FAISS_AVAILABLE and self.vector_store.index:
            faiss_path = str(save_dir / "faiss_index.bin")
            faiss.write_index(self.vector_store.index, faiss_path)
    ...
    def load_index(self, load_path: str):
        ...
        if metadata_path.exists():
            ...
            num_docs = len(metadata['documents'])
            num_chunks = len(metadata['chunks'])
            print(f"📚 Index chargé: {num_docs} docs, {num_chunks} chunks")
...
class RAGOptimizer:
    ...
    def optimize_chunk_size(
        ...
        for chunk_size in chunk_sizes:
            ...
            results[chunk_size] = {
                ...
                "efficiency_score": (
                    avg_quality / avg_time if avg_time > 0 else 0
                )
            }
        ...
        best_size = max(
            results.keys(), key=lambda x: results[x]["efficiency_score"]
        )
    ...
    def benchmark_retrieval_speed(self, num_queries: int = 100) -> Dict[str, Any]:
        ...
        print(
            f"⚡ Benchmark vitesse de récupération ({num_queries} requêtes)..."
        )

        # Générer des requêtes de test
        test_queries = [
            f"Question de test numéro {i} sur l'intelligence artificielle"
        ...
        return {
            ...
            "queries_per_second": (
                num_queries / sum(times) if sum(times) > 0 else 0
            ),
...
Vous pouvez stocker des entiers, des chaînes, des listes, et des dictionnaires.

## Fonctions
Les fonctions permettent de réutiliser le code.
Utilisez 'def' pour définir une fonction.
...
Le traitement du langage naturel permet aux machines de comprendre et générer
du texte humain.
...
## Quantization
La quantification réduit la précision des poids pour diminuer la taille du modèle
...
La distillation de connaissances transfère les connaissances d'un grand modèle
...
def run_rag_demo():
    ...
    for i, query in enumerate(demo_queries, 1):
        ...
        for j, chunk in enumerate(chunks[:2]):  # Montrer les 2 premiers
            ...
            print(f"   {j+1}. {source} (score: {score:.3f}) - {chunk_preview}...")
...
def benchmark_rag_vs_standard():
    ...
    for query in test_queries:
        ...
        print(f"   RAG: {rag_time:.3f}s, {rag_length} mots")
        print(f"   Standard: {standard_time:.3f}s, {standard_length} mots")
    
    # Résumé des performances
    num_rag_perf = len(results["rag_performance"])
    num_standard_perf = len(results["standard_performance"])
    avg_rag_time = sum(results["rag_performance"]) / num_rag_perf
    avg_standard_time = sum(results["standard_performance"]) / num_standard_perf

    print("\n📊 RÉSUMÉ COMPARATIF:")
    print(f"   RAG moyen: {avg_rag_time:.3f}s")
    print(f"   Standard moyen: {avg_standard_time:.3f}s")

    time_diff = avg_rag_time - avg_standard_time
    percentage_diff = (time_diff / avg_standard_time) * 100
    print(f"   Différence: {percentage_diff:+.1f}%")
...
def main():
    ...
    optimal_size = optimization_results['optimal_chunk_size']
    print(f"✅ Optimisation terminée: {optimal_size} tokens optimal")

    # Benchmark de vitesse
    speed_benchmark = optimizer.benchmark_retrieval_speed(50)
    queries_per_sec = speed_benchmark['queries_per_second']
    print(f"⚡ Vitesse de récupération: {queries_per_sec:.1f} requêtes/s")
