...
class OptimizationManager:
    ...
    def _initialize_components(self):
        ...
        # Context Optimizer
        if CONTEXT_OPT_AVAILABLE and opt_config.get("context_optimization", {}).get(
            "enabled", False
        ):
            try:
    ...
    def optimize_query(self, query: str, context: Dict[str, Any], model) -> str:
        ...
        if self.rag_pipeline:
            try:
                rag_response = self.rag_pipeline(query, model)
    ...
    def get_optimization_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "components_active": sum(
                [self.rag_pipeline is not None, self.context_optimizer is not None]
            ),
        }
