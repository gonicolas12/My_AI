...
class InputValidator:
    ...
    def validate_file_path(file_path: str) -> Dict[str, Any]:
        ...
        if len(str(path)) > 260:  # Limite Windows
            result["warnings"].append("Chemin très long (peut causer des problèmes sur Windows)")
    ...
    @staticmethod
    def validate_query(query: str, min_length: int = 1, max_length: int = 10000) -> Dict[str, Any]:
        """
        ...
        if len(query_clean) < min_length:
            result["valid"] = False
            result["errors"].append(f"Requête trop courte (minimum {min_length} caractères)")
        
        if len(query_clean) > max_length:
            result["valid"] = False
            result["errors"].append(f"Requête trop longue (maximum {max_length} caractères)")
    ...
    def validate_code(code: str, language: str) -> Dict[str, Any]:
        ...
        for pattern in patterns:
            if re.search(pattern, code, re.IGNORECASE):
                result["warnings"].append(f"Fonction potentiellement dangereuse détectée: {pattern}")
...
class SecurityValidator:
    ...
    @staticmethod
    def validate_file_upload(file_path: str, allowed_extensions: List[str] = None) -> Dict[str, Any]:
        """
