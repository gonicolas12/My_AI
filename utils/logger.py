...
class AILogger:
    ...
    def log_request(self, query: str):
        ...
        self.logger.info("Nouvelle requête: %s...", query[:100])
    ...
    def log_response(self, response_type: str, success: bool, processing_time: float):
        ...
        if success:
            ...
            self.logger.info(
                "Réponse %s générée en %.2fs", response_type, processing_time
            )
        else:
            self.metrics["errors"] += 1
            self.logger.error(
                "Échec de génération %s après %.2fs", response_type, processing_time
            )
    ...
    def log_error(self, error: Exception, context: str = ""):
        ...
        """
        self.metrics["errors"] += 1
        self.logger.error("Erreur %s: %s", context, str(error), exc_info=True)
