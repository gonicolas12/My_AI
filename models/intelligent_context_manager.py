...
class UltraIntelligentContextManager:
    ...
    def _generate_summary(self, text: str) -> str:
        ...
        return (
            ". ".join(sentences[:2]) + "." if len(sentences) > 1 else text[:100] + "..."
        )
    ...
    def get_context_for_query(self, query: str, max_tokens: int = 50000) -> str:
        ...
        print(
            f"📊 Contexte final: {total_tokens:,} tokens de {len(context_parts)//3} chunks"
        )
        return "\n".join(context_parts)
    ...
    def add_content_to_extended_context(
        self, content: str, content_type: str, importance: float = 1.0
    ):
    ...
    def generate_response_with_intelligent_context(
        ...
        relevant_context = self.context_manager.get_context_for_query(
            user_input, max_tokens=100000
        )
        ...
        if relevant_context:
            ...
            print("🚀 Utilisation du contexte étendu pour la réponse")
