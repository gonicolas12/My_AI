...
class ConversationMemory:
    ...
    def _update_context_cache(self, entry: ConversationEntry) -> None:
        ...
        if len(self.context_cache["recent_topics"]) > 10:
            self.context_cache["recent_topics"] = self.context_cache["recent_topics"][
                -10:
            ]
    ...
    def has_similar_recent_question(
        self, user_message: str, threshold: float = 0.7
    ...
    def store_document_content(self, filename: str, content: str) -> None:
        ...
        self.stored_documents[filename] = {
            ...
            "order_index": (
                len(self.document_order) - 1
                if filename not in self.stored_documents
                else self.stored_documents[filename].get("order_index", 0)
            ),
        }

        print(
            f"✅ Document '{filename}' stocké en mémoire ({len(content.split())} mots) - Position: {self.stored_documents[filename]['order_index'] + 1}"
        )
    ...
    def get_document_by_reference(self, reference: str) -> Dict[str, Any]:
        ...
        if any(
            ref in reference_lower
            for ref in ["premier", "première", "first", "1er", "1ère"]
        ):
        ...
        # Références au dernier document
            ...
            ref in reference_lower
            for ref in ["dernier", "dernière", "last", "récent", "recent"]
        ):
        ...
        # Références au précédent document
            ...
            ref in reference_lower
            for ref in ["précédent", "precedent", "previous", "avant", "d'avant"]
        ):
        ...
        # Références par numéro
            ...
            ref in reference_lower
            for ref in ["deuxième", "deuxieme", "second", "2ème", "2eme"]
        ):
            if len(self.document_order) >= 2:
            ...
            ref in reference_lower
            for ref in ["troisième", "troisieme", "third", "3ème", "3eme"]
        ):
        ...
        if self.document_order:
            last_filename = self.document_order[-1]
            print(
                f"🔄 Aucune référence spécifique, retour du dernier: '{last_filename}'"
            )
            return {last_filename: self.stored_documents[last_filename]}
