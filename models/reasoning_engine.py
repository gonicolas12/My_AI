...
class ReasoningEngine:
    ...
    def _load_reasoning_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reasoning_chains": {
                "cause_effect": ["cause", "effet", "conséquence", "résultat", "provoque"],
                "comparison": ["plus", "moins", "mieux", "pire", "similaire", "différent"],
    ...
    def analyze_reasoning(self, text: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        reasoning_analysis = {
            ...
            "required_strategy": self._suggest_strategy(text_lower, context)
        }
    ...
    def _identify_question_type(self, text: str) -> Optional[str]:
        ...
        for question_type, keywords in self.reasoning_patterns["question_types"].items():
            for keyword in keywords:
                if keyword in text:
    ...
    def _suggest_strategy(self, text: str, context: Dict[str, Any]) -> str:
    ...
    def solve_problem(self, problem: str, context: Dict[str, Any]) -> str:
        """Résout un problème en utilisant le raisonnement"""
        analysis = self.analyze_reasoning(problem, context)
        ...
        concrete_approach = self._generate_concrete_approach(problem, analysis)
        solution_text += f"\n💡 **Approche concrète :**\n{concrete_approach}"
        
        return solution_text
    
    def _generate_concrete_approach(self, problem: str, analysis: Dict[str, Any]) -> str:
    ...
    def generate_explanation(self, topic: str, context: Dict[str, Any]) -> str:
    ...
    def process_query(self, query: str) -> str:
        ...
        try:
            ...
            if any(word in query.lower() for word in ["problème", "résoudre", "comment", "pourquoi", "solution"]):
                return self.solve_problem(query, context)
            elif any(word in query.lower() for word in ["expliquer", "qu'est-ce que", "définir", "concept"]):
                return self.generate_explanation(query, context)
            else:
                # Analyse de raisonnement générale
                analysis = self.analyze_reasoning(query, context)
